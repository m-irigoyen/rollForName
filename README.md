...
Two new things have appeared ! First, a roll : `[1d5]`. All rolls are written between brackets `[]`. The syntax is pretty easy : `[1d5]` reads "throw 1 dice that has 5 faces". 
Since this is a computer program, and we don't care about stuff like physics in here, you can use the number of faces you want. Of course, you can do complicated rolls by adding modifiers, like `[1d5+2]`. That will throw a d5, then add 2 to the result. 
...
All variables have a name, preceeded by a dollar $, and the whole thing is surrounded by parentheses. So if you write `($test)`, you are refering to the variable named "test". You can't put spaces in a variable name, and its a good idea to keep them short and meaningful.
...
You can put as many as you want in there, separated by a comma. For example : 'required : "table1", "table2" , "table3", "table4"	// etc..'
...
All you need to do is change the "t" for a "g", and you can call on a generator instead! Note that the "character" generator doesn't have a "required tables" line. That's because those tables already are loaded by the other generators.
